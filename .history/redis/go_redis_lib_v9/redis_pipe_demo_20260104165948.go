...
// 或者，也可以使用Pipelined 方法，它会在函数退出时调用 Exec。
// 效果等价于上面的 redisPipelineDemo 方法。
func redisPipelinedDemo() error {
	ctx, cancel := context.WithTimeout(context.Background(), 500*time.Millisecond) // 设置超时时间
	defer cancel()                                                                 // 释放资源

	_, err := rdb.Pipelined(ctx, func(pipe redis.Pipeliner) error {
		pipe.Incr(ctx, "pipelined_counter1")
		pipe.Incr(ctx, "pipelined_counter2")
		pipe.Expire(ctx, "pipelined_counter1", time.Hour)
