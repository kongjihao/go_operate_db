...
func scanAllKeys() ([]string, error) {
	...
	for {
		...
		var err error
		scanKeys, cursor, err = rdb.Scan(ctx, cursor, "prefix:*", 10).Result() // 以prefix:开头的，每次扫描10个key
		if err != nil {
			return nil, err
		}
...
// 针对这种需要遍历大量key的场景，go-redis中提供了一个简化方法——Iterator，简化了 Scan 的使用，其使用示例如下。
func scanAllKeysWithIterator() ([]string, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 500*time.Millisecond) // 设置超时时间
	defer cancel()                                                                 // 释放资源

	var keys []string
	iter := rdb.Scan(ctx, 0, "*:suffix", 10).Iterator() // 以:suffix结尾的，每次扫描10个key
	for iter.Next(ctx) {
...
func delKeysByMatch(match string, timeout time.Duration) {
	...
	for iter.Next(ctx) {
		err := rdb.Del(ctx, iter.Val()).Err() // 删除key
		if err != nil {
			panic(err)
		}
	}
...
// 对于 Redis 中的 set、hash、zset 数据类型，go-redis 也支持类似的遍历方法。
// iter := rdb.SScan(ctx, "set-key", cursor, "prefix:*", count).Iterator()
// iter := rdb.HScan(ctx, "hash-key", cursor, "prefix:*", count).Iterator()
// iter := rdb.ZScan(ctx, "sorted-hash-key", cursor, "prefix:*", count).Iterator()
